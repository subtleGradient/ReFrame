...
"production" !== process.env.NODE_ENV &&
  ((module.exports = function ($$$config) {
    ...
    function serializeServerReference(request, serverReference) {
      ...
      existingId = getServerReferenceBoundArguments(request.bundlerConfig, serverReference)
      existingId = null === existingId ? null : Promise.resolve(existingId)
      var id = getServerReferenceId(request.bundlerConfig, serverReference),
        location = null,
        error = getServerReferenceLocation(request.bundlerConfig, serverReference)
    ...
    function renderModelDestructive(request, task, parent, parentPropertyName, value) {
      ...
      if ("function" === typeof value) {
        if (isClientReference(value)) return serializeClientReference(request, parent, parentPropertyName, value)
        if (isServerReference(value)) return serializeServerReference(request, value)
        if (
          void 0 !== request.temporaryReferences &&
    ...
    function logRecoverableError(request, error, task) {
      ...
      try {
        ...
        var errorDigest = null !== task ? callWithDebugContextInDEV(request, task, onError, error) : onError(error)
      } finally {
    ...
    function flushCompletedChunks(request, destination) {
      ...
      0 === request.pendingChunks && ((request.status = CLOSED), close(destination), (request.destination = null))
    }
    ...
    var isClientReference = $$$config.isClientReference,
      ...
      getServerReferenceBoundArguments = $$$config.getServerReferenceBoundArguments,
      getServerReferenceLocation = $$$config.getServerReferenceLocation,
      frameRegExp = /^ {3} at (?:(.+) \((?:(.+):(\d+):(\d+)|<anonymous>)\)|(?:async )?(.+):(\d+):(\d+)|<anonymous>)$/,
